...
esp_err_t bsp_display_new(const bsp_display_config_t *config, esp_lcd_panel_handle_t *ret_panel, esp_lcd_panel_io_handle_t *ret_io)
{
    ...
    const spi_bus_config_t buscfg = AXS15231B_PANEL_BUS_QSPI_CONFIG(
        ...
        config->max_transfer_sz); // 48KB chunks ✓
    ...
    esp_lcd_panel_disp_on_off(*ret_panel, true); // Turn ON
    esp_lcd_panel_swap_xy(*ret_panel, false);    // ADD
    esp_lcd_panel_mirror(*ret_panel, false, false); // ADD
...
static lv_display_t *bsp_display_lcd_init(const bsp_display_cfg_t *cfg)
{
    ...
    // Your existing panel creation code stays here
    const bsp_display_config_t bsp_disp_cfg = {

        .max_transfer_sz = EXAMPLE_LCD_QSPI_V_RES * 10 * sizeof(uint16_t), // 48KB

        .tear_cfg = {0},
    };
    bsp_display_new(&bsp_disp_cfg, &panel_handle, &io_handle);

    // ←←← OFFICIAL PORT CONFIG (this replaces your old lvgl_port_add_disp)
    const lvgl_port_display_cfg_t disp_cfg = {
        .io_handle = io_handle,
        .panel_handle = panel_handle,
        .buffer_size = EXAMPLE_LCD_QSPI_H_RES* EXAMPLE_LCD_QSPI_V_RES, // full buffer
        .double_buffer = true,
        .hres = EXAMPLE_LCD_QSPI_H_RES, // 320
        .vres = EXAMPLE_LCD_QSPI_V_RES, // 480
        .color_format = LV_COLOR_FORMAT_RGB565,
        .rotation = {.swap_xy = false, .mirror_x = false, .mirror_y = false},
        .flags = {
            .buff_spiram = true,
            .buff_dma = true,
            .swap_bytes = true, // change to false if red/blue swapped
            .full_refresh = true,
            .sw_rotate = true,
        }};
    ...
    // Apply your 90° rotation
    lv_display_set_rotation(disp, cfg->rotate);
...
// Add this BEFORE bsp_display_indev_init() - around line 750
static void touch_read_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    uint16_t x[2] = {0}, y[2] = {0}, strength[2] = {0};
    uint8_t count = 0;

    esp_lcd_touch_read_data(tp);
    bool touched = esp_lcd_touch_get_coordinates(tp, x, y, strength, &count, 2);
...
static lv_indev_t *bsp_display_indev_init(const bsp_display_cfg_t *config, lv_disp_t *disp)
{
    ...
    lv_indev_set_read_cb(indev, touch_read_cb); // ← Function pointer!
...
{
    // BACKLIGHT FIRST - BEFORE ANYTHING ELSE!
    bsp_display_brightness_init();
    bsp_display_brightness_set(100); // 100% NOW!
    ...
    // SKIP TOUCH FOR NOW
    disp_indev = NULL; // Disable touch completely
    return disp;
}
